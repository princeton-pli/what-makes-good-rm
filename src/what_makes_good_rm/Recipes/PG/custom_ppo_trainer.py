# Adapted based on the PPOTrainer implementation in the TRL library https://huggingface.co/docs/trl/en/index
...
from what_makes_good_rm.Arguments.args import CustomPPOConfig
...
class CustomPPOTrainer(Trainer):
    """
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if self.is_deepspeed_enabled:
            state_dict = {
                name.removeprefix('policy.'): param for name, param in state_dict.items() if name.startswith('policy.')
            }
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                if args.scale_reward_for_frac_prompts > 0:
                    ...
                    scores[should_scale_reward] *= args.reward_scale_factor

                # 5. whiten rewards
                if args.whiten_rewards:
                    # Whiten rewards across the batch, including the KL term in the reward (original TRL implementation)
                    ...
                    # Whiten rewards  across the batch, excluding the KL term.
                        ...
                        normalized_scores = (scores - scores_mean) * torch.rsqrt(scores.var() + 1e-8)
                        normalized_scores += scores_mean
                        rewards[[actual_start, actual_end]] += normalized_scores
                        rewards = torch.masked_fill(rewards, padding_mask_p1, 0)
