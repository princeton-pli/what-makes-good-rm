...
class CustomRLOOTrainer(RLOOTrainer):
    """
    Adapted from https://github.com/huggingface/trl/blob/main/trl/trainer/rloo_trainer.py
    """
    _tag_names = ["trl", "rloo"]
    ...
    def __init__(
        ...
        if hasattr(self.reward_model.config, "reward_shift") and hasattr(self.reward_model.config, "reward_scale"):
            self.reward_shift = self.reward_model.config.reward_shift
            self.reward_scale = self.reward_model.config.reward_scale
            logger.info(f"Found 'reward_shift' and 'reward_scale' in the reward model config. "
                        f"Using 'reward_shift'={self.reward_shift} and 'reward_scale'={self.reward_scale}")
        else:
            logger.warning("No 'reward_shift' or 'reward_scale' found in the reward model config. "
                           "Setting default values of reward_shift=0.0 and reward_scale=1.0.")
    ...
    def __get_reward_with_correct_tokenizer(self, batch_prompts, query_tokens, response_tokens, pad_token_id,
        ...
        score = (score + self.reward_shift) * self.reward_scale
        return score
    ...
    def train(self):
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                maybe_normalized_scores = torch.clone(scores).reshape(args.rloo_k, -1)
                ...
                if args.scale_reward_for_frac_prompts > 0:
                    ...
                    maybe_normalized_scores[:, should_scale_reward] *= args.reward_scale_factor

                # Normalize per prompt
                if args.per_prompt_normalize_rewards:
                    maybe_normalized_scores *= torch.rsqrt(maybe_normalized_scores.var(0) + 1e-8)

                maybe_normalized_scores = maybe_normalized_scores.flatten()
                rlhf_reward = maybe_normalized_scores + non_score_reward

                if args.whiten_rewards:
                    # Whiten rewards across the batch, including the KL term in the reward.
                    # Mimics original functionality in PPOTrainer, and accordingly keeps the original mean
                    if not args.whiten_rewards_without_kl:
                        rlhf_reward_mean = rlhf_reward.mean()
                        rlhf_reward = (rlhf_reward - rlhf_reward_mean) * torch.rsqrt(rlhf_reward.var() + 1e-8)
                        rlhf_reward += rlhf_reward_mean
                    # Whiten rewards  across the batch, excluding the KL term.
                    else:
                        maybe_normalized_scores_mean = maybe_normalized_scores.mean()
                        maybe_normalized_scores = (maybe_normalized_scores - maybe_normalized_scores_mean) * torch.rsqrt(
                            maybe_normalized_scores.var() + 1e-8
                        )
                        maybe_normalized_scores += maybe_normalized_scores_mean
                        rlhf_reward = maybe_normalized_scores + non_score_reward
            ...
            with torch.no_grad():
                ...
                self.log(metrics)
            del kl, mean_kl, mean_entropy, scores, maybe_normalized_scores
            ...
            if args.early_stopping and args.target_kl:
                if metrics["objective/kl"] > 1.5 * args.target_kl:
                    accelerator.print("===early stopping due to KL stopping criteria===")
                    break
